...
public class MemberPutApiTest {
    ...
    @Test
    public void modify_member_put() throws Exception {

        given(service.modifyMemberDetailsPut(eq(1L) , any(MemberModificationRequest.class)))
                .willReturn(new MemberDTO(
                        1L , "xxx7949@naver.com" ,"명사면" ,
                        LocalDateTime.of(2024,01,12, 00,21,40),
                        LocalDateTime.of(2024,01,12 ,00,21,40)
                ));

        MemberModificationRequest memberSignupRequest = new MemberModificationRequest("명사면");


        mvc.perform(put("/api/members/modify/{memberId}" , 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(memberSignupRequest)))
                .andDo(document(
                        "members/modifyPutApiMemberName",
